...
normalise(exactly_one_pos(P), Res) :- 
    (length(P, 1) -> 
        get_element(0, P, B), 
        normalise(B, W), 
        append(W, [], Res), 
        ...
        flatten(Test, WTF), 
        get_element(0, P, T), 
        exactly_one(WTF, Temp, T), 
        append([WTF], Temp, Res), 
        !
    ).

normalise(min_one_pos(P), Res) :- 
    min_one(P, Temp), 
    flatten(Temp, What), 
    append([What], [], Res), 
    !.
...
exactly_one(T, Res, Temp) :- 
    ...
    get_element(0, T, W), 
    (A \== W -> 
        append([[not(W), not(A)]], [], Res); 
        append([not(W)], [], Res)
    ), 
    !.
exactly_one([H,X|T], [Test|Res], Temp) :- 
    Test = [not(H), not(X)], 
    exactly_one([X|T], Res, Temp).
